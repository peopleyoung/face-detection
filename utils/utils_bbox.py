...
def retinaface_correct_boxes(result, input_shape, image_shape):
    ...
    scale_for_boxs = np.array([scale[1], scale[0], scale[1], scale[0]])
    scale_for_landmarks = np.array([
        scale[1], scale[0], scale[1], scale[0], scale[1], scale[0], scale[1],
        scale[0], scale[1], scale[0]
    ])

    offset_for_boxs = np.array([offset[1], offset[0], offset[1], offset[0]])
    ...
    result[:, :4] = (result[:, :4] -
                     np.array(offset_for_boxs)) * np.array(scale_for_boxs)
